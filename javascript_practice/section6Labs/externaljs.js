function startConversation() {
    ...
    if (!isNaN(parsedNum1)) {
        ...
        if (!isNaN(parsedNum2)) {
            alert("You entered: " + parsedNum2);


            let result = addNumbers(parsedNum1, parsedNum2);


            alert("The result of adding " + parsedNum1 + " and " + parsedNum2 + " is: " + result);
            document.write("<p>The result of the addNumbers() function is: <strong>" + result + "</strong></p>");
        } else {
            alert("Invalid input for the second number. Please enter a numerical value.");
        }
    } else {
        alert("Invalid input for the first number. Please enter a numerical value.");
    }
}